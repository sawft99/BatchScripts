...
:: Firefox cleanup
if exist "%systemdrive%\Users\" for /D %%a in ("%systemdrive%\Users\*") (
...
if exist "%ProgramFiles%\Nvidia Corporation\Installer2" (
	rmdir /s /q "%ProgramFiles%\Nvidia Corporation\Installer2"
)
if exist "%ALLUSERSPROFILE%\NVIDIA Corporation\NetService" ( 
	del /f /q "%ALLUSERSPROFILE%\NVIDIA Corporation\NetService\*.exe"
)
:: Clear Windows Search Temp Data + Index
if exist "%ALLUSERSPROFILE%\Microsoft\Search\Data\Temp" (
	rmdir /s /q "%ALLUSERSPROFILE%\Microsoft\Search\Data\Temp"
)
echo.
...
if exist %SystemDrive%\i386 (
	rmdir /S /Q %SystemDrive%\i386
)
...
if exist "%ALLUSERSPROFILE%\Microsoft\Windows\WER\ReportArchive" (
	rmdir /s /q "%ALLUSERSPROFILE%\Microsoft\Windows\WER\ReportArchive"
)
if exist "%ALLUSERSPROFILE%\Microsoft\Windows\WER\ReportQueue" (
	rmdir /s /q "%ALLUSERSPROFILE%\Microsoft\Windows\WER\ReportQueue"
)

:: Clear Windows Defender Scan Results
if exist "%ALLUSERSPROFILE%\Microsoft\Windows Defender\Scans\History\Results\Quick" (
	rmdir /s /q "%ALLUSERSPROFILE%\Microsoft\Windows Defender\Scans\History\Results\Quick"
)
if exist "%ALLUSERSPROFILE%\Microsoft\Windows Defender\Scans\History\Results\Resource" (
	rmdir /s /q "%ALLUSERSPROFILE%\Microsoft\Windows Defender\Scans\History\Results\Resource"
)
...
REM	if exist %WINDIR%\system32\dllcache\tourstrt.exe (
REM		del %WINDIR%\system32\dllcache\tourstrt.exe 2>NUL
REM	)
REM	if exist %WINDIR%\system32\dllcache\tourW.exe 
REM		del %WINDIR%\system32\dllcache\tourW.exe 2>NUL
REM	)
REM	if exist %WINDIR%\Help\Tours (
REM		rmdir /S /Q %WINDIR%\Help\Tours 2>NUL
REM	)
...
REM 	if exist %WINDIR%\Media (
REM		takeown /f %WINDIR%\Media /r /d y 2>NUL && echo.
REM	)
REM	if exist %WINDIR%\Media (
REM		icacls %WINDIR%\Media /grant administrators:F /t  && echo.
REM	)
REM	if exist %WINDIR%\Media (
REM		rmdir /S /Q %WINDIR%\Media 2>NUL
REM	)
REM )
